...
class Reaction(BaseProcedures):
    ...
    def run_reactions(self, rxn_steps: Dict = None):
        ...
        # print(self.reaction_steps)
        for i in range(0, len(rxn_steps)):
            # if i+1 <= 3:
            #     continue
                # input('add water then press enter')

            self.run_reaction(rxn_steps[f"{i + 1}"])


rxn = Reaction()
rxn.generate_experiment_details(num_reactions = 8, temp = 60, reaction_time = 6)
# with open(Path.cwd().parent.parent/'data'/'experiment_plan.json', 'r') as f:
#    steps = json.load(f)
# print(steps["20"])
# rxn.run_reactions()
# rxn.generate_rinse_details(num_reactions=10)
rxn.run_reactions()
