...
class BaseProcedures(object):
    ...
    def heat_and_stir(self,
        ...
        if reaction_time >  0:          # for reaction_time = 0, heat inf)
...
class Reaction(BaseProcedures):
    def __init__(self):
        ...
        self.generate_experiment_details()
        self.time_log={"0": time.time()}
        
    def generate_experiment_details(self, num_reactions: int, temp: float):
        self.num_points = num_reactions
        self.temp = temp
    ...
    def run_reaction(self):
        self.heat_and_stir(reaction_time=0, temp=self.temp, rpm=800)
        self.inject_samples()
